#This program creates a sudoku grid and solves it.
...
def generate_sudoku(mask_rate=0.5):
    ...
    while True:
        ...
        try:
            for r in range(1, n):
                for c in range(n):
                    col_rest = np.setdiff1d(rg, m[:r, c]) #Return the unique values in rg, that are not in m[:r, c].
                    row_rest = np.setdiff1d(rg, m[r, :c])
                    avb1 = np.intersect1d(col_rest, row_rest) #Return the sorted, unique values that are in both of the input arrays
                    sub_r, sub_c = r//3, c//3
